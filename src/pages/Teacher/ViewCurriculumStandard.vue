...
const getAllCourseStandards = async () => {
    try {
        const response = await axios.get('/api/teacher/get-all-course-standards');
...
const handleFileUpload = async (event) => {
    ...
    if (file) {
        ...
        if (currentItem.value) {
            try {
                const response = await axios.put(`/api/teacher/update-course-standard/${currentItem.value.id}`, formData,
            ...
            // 新建课标时
            try {
                const response = await axios.post('/api/teacher/create-course-standard', formData,
                        { headers: { 'Content-Type': 'multipart/form-data' } });
                if (response.status === 200) {
                    await getAllCourseStandards();
...
const viewFile = async (item) => {
    try {
        const response = await axios.get(`/api/teacher/query-course-standard/${item.id}`, {
            responseType: 'blob',
        });
        if (response.status === 200) {
